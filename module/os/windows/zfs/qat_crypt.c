    ...
    crypto_key_t *key, uint64_t crypt, uint32_t enc_len)
{
	...
	if (aad_len > 0) {
		...
		if (status != CPA_STATUS_SUCCESS)
			goto fail;
		memset(op_data.pAdditionalAuthData, aad_buf, aad_len);
	}
	...
	memset(op_data.pIv, iv_buf, ZIO_DATA_IV_LEN);
	/* if dir is QAT_DECRYPT, copy digest_buf to pDigestResult */
	if (dir == QAT_DECRYPT)
		memset(op_data.pDigestResult, digest_buf, ZIO_DATA_MAC_LEN);
	...
	if (dir == QAT_ENCRYPT) {
		...
		memset(digest_buf, op_data.pDigestResult, ZIO_DATA_MAC_LEN);
...
qat_checksum(uint64_t cksum, uint8_t *buf, uint64_t size, zio_cksum_t *zcp)
{
	...
	memset(zcp, digest_buffer, sizeof (zio_cksum_t));
