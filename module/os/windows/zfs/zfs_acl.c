    ...
    zfs_fuid_info_t **fuidp, cred_t *cr)
{
	...
	for (i = 0; i != aclcnt; i++) {
		...
		case ACE_SYSTEM_ALARM_OBJECT_ACE_TYPE:
			...
			memset(zobjacep->z_object_type,
			    aceobjp->a_obj_type,
			    sizeof (aceobjp->a_obj_type));
			memset(zobjacep->z_inherit_type,
    ...
    void *datap, int filter)
{
	...
	while ((zacep = zfs_acl_next_ace(aclp, zacep,
		...
		case ACE_SYSTEM_ALARM_OBJECT_ACE_TYPE:
			...
			memset(objacep->a_obj_type,
			    zobjacep->z_object_type,
			    sizeof (zobjacep->z_object_type));
			memset(objacep->a_inherit_obj_type,
    ...
    boolean_t will_modify)
{
	...
	if (!zp->z_is_sa) {
		if (znode_acl.z_acl_extern_obj) {
			...
			memset(aclnode->z_acldata, znode_acl.z_ace_data,
			    aclnode->z_size);
		}
	} else {
	...
	if (zp->z_is_sa) { /* the easy case, just update the ACL attribute */
		...
		if (aclp->z_acl_bytes > ZFS_ACE_SPACE) {
			...
			for (aclnode = list_head(&aclp->z_acl); aclnode;
				...
				if (aclnode->z_ace_count == 0)
					continue;
				memset(start, aclnode->z_acldata,
				    aclnode->z_size);
