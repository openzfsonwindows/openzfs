...
InitVpb(__in PVPB Vpb, __in PDEVICE_OBJECT VolumeDevice, mount_t *zmo)
{
	if (Vpb != NULL) {
		...
		Vpb->VolumeLabelLength = MIN(zmo->name.Length, sizeof (Vpb->VolumeLabel));

		RtlCopyMemory(Vpb->VolumeLabel, zmo->name.Buffer, Vpb->VolumeLabelLength);

		LARGE_INTEGER systemTime;
		KeQuerySystemTime(&systemTime);
		Vpb->SerialNumber = systemTime.LowPart ^ systemTime.HighPart;
...
#if 1
	if (taskq_dispatch(system_taskq, io_verify_volume_impl,
	    dcb->FunctionalDeviceObject, TQ_SLEEP) == 0)
		IoVerifyVolume(dcb->FunctionalDeviceObject, FALSE);
#endif // either this, or that we removed attach
}
    ...
    PDEVICE_OBJECT PhysicalDeviceObject, PDEVICE_OBJECT AddDeviceObject)
{
	...
	// This one crashes. Makes a device loop
