...
#define	P2BOUNDARY(off, len, align) \
				...
				((uint64_t)len) - 1)) > ((uint64_t)align) - 1)
...
#define	P2PHASEUP_TYPED(x, align, phase, type)  \
	((type)((uint64_t)phase) - (((type)((uint64_t)phase) - \
	(type)((uint64_t)x)) & -(type)((uint64_t)align)))
#define	P2CROSS_TYPED(x, y, align, type)        \
	(((type)((uint64_t)x) ^ (type)((uint64_t)y)) > \
	(type)((uint64_t)align) - 1)
...
#define	P2PHASEUP(x, align, phase)	\
	(((uint64_t)phase) - ((((uint64_t)phase) - \
	((uint64_t)x)) & -((uint64_t)align)))
