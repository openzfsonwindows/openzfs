...
#define	KMEM_SLAB_OFFSET(sp, buf)	\
	((uint32_t)((uintptr_t)(buf) - (uintptr_t)((sp)->slab_base)))
...
#define	KMEM_CACHE_SIZE(ncpus)	\
	offsetof(kmem_cache_t, cache_cpu[ncpus])

	/* Offset from kmem_cache->cache_cpu for per cpu caches */
#define	KMEM_CPU_CACHE_OFFSET(cpuid)	\
	offsetof(kmem_cache_t, cache_cpu[cpuid]) - \
	offsetof(kmem_cache_t, cache_cpu)
...
typedef struct kmem_defrag {
	...
	uint32_t	kmd_deadcount;		/* # of slabs in kmd_deadlist */
	...
	uint32_t	kmd_slabs_sought;	/* reclaimable slabs sought */
	uint32_t	kmd_slabs_found;	/* reclaimable slabs found */
	uint32_t	kmd_tries;		/* nth scan interval counter */
	/*
...
struct kmem_cache {
	...
	uint64_t no_vba_success; /* successful calls with KM_NO_VBA set */
	...
	uint32_t	cache_bufsize;	/* object size */
	uint32_t	cache_align;	/* object alignment */
	...
	uint32_t	cache_chunksize; 	/* buf + alignment [+ debug] */
	uint32_t	cache_slabsize;		/* size of a slab */
	uint32_t	cache_maxchunks; 	/* max buffers per slab */
	uint32_t	cache_bufctl;		/* buf-to-bufctl distance */
	uint32_t	cache_buftag;		/* buf-to-buftag distance */
	uint32_t	cache_verify;		/* bytes to verify */
	uint32_t	cache_contents;		/* bytes of saved content */
	uint32_t	cache_color;		/* next slab color */
	uint32_t	cache_mincolor;		/* maximum slab color */
	uint32_t	cache_maxcolor;		/* maximum slab color */
	uint32_t	cache_hash_shift;	/* get to interesting bits */
	uint32_t	cache_hash_mask;	/* hash table mask */
	list_t		cache_complete_slabs;	/* completely allocated slabs */
	uint32_t	cache_complete_slab_count;
	avl_tree_t	cache_partial_slabs;	/* partial slab freelist */
	uint32_t	cache_partial_binshift;	/* for AVL sort bins */
